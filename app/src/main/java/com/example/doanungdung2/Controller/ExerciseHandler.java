...
public class ExerciseHandler extends SQLiteOpenHelper {
    ...
    public boolean updateExercises(Exercise e) {
        ...
        try {
            ...
            contentValues.put(soCau, e.getSoCau());
    ...
    public boolean checkExerciseByNameAndCode(String tenBaiTapInput, String maBaiTapInput) {
        boolean exists = false;

        SQLiteDatabase sqLiteDatabase = SQLiteDatabase.openDatabase(PATH, null, SQLiteDatabase.OPEN_READONLY);
        String query = "SELECT * FROM " + TABLE_NAME + " WHERE " + tenBaiTap + " = ? AND " + maBaiTap + " != ?";
        Cursor cursor = sqLiteDatabase.rawQuery(query, new String[]{tenBaiTapInput, maBaiTapInput});
    ...
    @SuppressLint("Range")
    public String getNameExerciseByCode(String maBaiTapInput) {
        String name = "";
        SQLiteDatabase sqLiteDatabase = SQLiteDatabase.openDatabase(PATH, null, SQLiteDatabase.OPEN_READONLY);
        String query = "Select TenBaiTap From " + TABLE_NAME + " Where MaBaiTap = ?";
        Cursor cursor = sqLiteDatabase.rawQuery(query, new String[]{maBaiTapInput});
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                // Lấy giá trị của cột TenBaiTap
                name = cursor.getString(cursor.getColumnIndex("TenBaiTap"));
            }
            cursor.close(); // Đảm bảo đóng cursor sau khi sử dụng
        }
        sqLiteDatabase.close(); // Đảm bảo đóng cơ sở dữ liệu sau khi xong
        return name; // Trả về tên bài tập
    }
    @SuppressLint("Range")
    public String checkAssignmentHaveCode(String tenBaiTapInput)
    {
        String maBaiTap = "";
        SQLiteDatabase sqLiteDatabase = SQLiteDatabase.openDatabase(PATH, null, SQLiteDatabase.OPEN_READONLY);
        String query = "Select MaBaiTap from " + TABLE_NAME + " Where TenBaiTap = ?";
        Cursor cursor = sqLiteDatabase.rawQuery(query, new String[]{tenBaiTapInput});
        if (cursor != null)
        {
            if (cursor.moveToFirst())
            {
                maBaiTap = cursor.getString(cursor.getColumnIndex("MaBaiTap"));
            }
            cursor.close();
        }
        sqLiteDatabase.close();
